...
def _detect_pyc_compile() -> Path:
    ...
    raise SystemExit("missing pyc-compile (set PYC_COMPILE=... or build it with: flows/scripts/pyc build)")
...
def _cmd_testgen(args: argparse.Namespace) -> int:
    ...
    common_compile_args = [f"--logic-depth={int(args.logic_depth)}", f"--sim-mode={args.sim_mode}"]

    # Compile targets into the same out-dir.
    if args.sim_mode != "cpp-only":
        subprocess.run([str(pyc_compile), str(pyc_path), "--emit=verilog", f"--out-dir={out_dir}", *common_compile_args], check=True)
    subprocess.run([str(pyc_compile), str(pyc_path), "--emit=cpp", f"--out-dir={out_dir}", *common_compile_args], check=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    testgen.add_argument("python_file", help="Python source defining `@module build(...)` and tb(t: Tb)")
    ...
    testgen.add_argument(
        "--sim-mode",
        choices=["default", "cpp-only"],
        default="default",
        help="Simulation emission mode. `cpp-only` skips Verilog generation.",
    )
    testgen.add_argument(
        "--logic-depth",
        type=int,
        default=32,
        help="Strict max combinational logic depth passed to pyc-compile (default: 32).",
    )
