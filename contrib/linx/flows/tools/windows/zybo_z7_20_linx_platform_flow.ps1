...
$tcl = switch ($Core) {
  "InOrder" { Join-Path $repo "boards\\zybo_z7_20\\vivado\\build_zybo_linx_platform_inorder.tcl" }
  "OOO"     { Join-Path $repo "boards\\zybo_z7_20\\vivado\\build_zybo_linx_platform_ooo.tcl" }
}
if (-not (Test-Path $tcl)) {
  throw "Missing build script: $tcl"
}

Write-Host ("[zybo-platform] repo:   {0}" -f $repo)
Write-Host ("[zybo-platform] core:   {0}" -f $Core)
Write-Host ("[zybo-platform] vivado: {0}" -f $VivadoBat)
Write-Host ("[zybo-platform] tcl:    {0}" -f $tcl)
