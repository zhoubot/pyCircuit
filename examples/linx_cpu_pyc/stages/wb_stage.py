...
def build_wb_stage(
    ...
    *,
    ...
    op_c_bstart_cond = memwb.op.eq(c(OP_C_BSTART_COND, width=6))
    op_bstart_call = memwb.op.eq(c(OP_BSTART_STD_CALL, width=6))
    op_c_bstop = memwb.op.eq(c(OP_C_BSTOP, width=6))

    op_is_start_marker = op_c_bstart_std | op_c_bstart_cond | op_bstart_call
    op_is_boundary = op_is_start_marker | op_c_bstop

    br_is_fall = state.br_kind.eq(c(BK_FALL, width=2))
    br_is_cond = state.br_kind.eq(c(BK_COND, width=2))
    br_is_call = state.br_kind.eq(c(BK_CALL, width=2))
    br_is_ret = state.br_kind.eq(c(BK_RET, width=2))

    br_target_pc = state.br_base_pc + state.br_off
    br_target_pc = br_is_ret.select(state.commit_tgt, br_target_pc)

    br_take = br_is_call | br_is_ret | (br_is_cond & state.commit_cond)
    ...
    op_c_setc_eq = memwb.op.eq(c(OP_C_SETC_EQ, width=6))
    op_c_setc_tgt = memwb.op.eq(c(OP_C_SETC_TGT, width=6))

    commit_cond_next = state.commit_cond
    commit_tgt_next = state.commit_tgt
    ...
    br_kind_next = state.br_kind
    br_base_next = state.br_base_pc
    br_off_next = state.br_off
    ...
    br_base_next = (do_wb & op_is_boundary & br_take).select(state.pc, br_base_next)
    ...
    br_base_next = (enter_new_block & op_c_bstart_cond).select(state.pc, br_base_next)
    br_off_next = (enter_new_block & op_c_bstart_cond).select(memwb.value, br_off_next)
    ...
    br_base_next = (enter_new_block & op_bstart_call).select(state.pc, br_base_next)
    br_off_next = (enter_new_block & op_bstart_call).select(memwb.value, br_off_next)

    # C.BSTART.STD BrType: fall-through (BrType=1) or return (BrType=7).
    brtype = memwb.value.trunc(width=3)
    ...
    br_base_next = (enter_new_block & op_c_bstart_std).select(state.pc, br_base_next)
    ...
    do_reg_write = do_wb & (~wb_is_store) & (~memwb.regdst.eq(c(REG_INVALID, width=6)))

    do_clear_hands = do_wb & op_is_start_marker
    do_push_t = do_wb & memwb.op.eq(c(OP_C_LWI, width=6))

    do_push_t = do_push_t | (do_reg_write & memwb.regdst.eq(c(31, width=6)))
    do_push_u = do_reg_write & memwb.regdst.eq(c(30, width=6))
