...
def build_tau(
    ...
    *,
    ...
    op_sub = payload.out()[0]
    src_a = payload.out()[16:32].zext(width=64)
    src_b = payload.out()[32:48].zext(width=64)
    sub_res = src_a + ((~src_b) + c(1, width=64))
    tau_res = op_sub.select(sub_res, src_a + src_b)
